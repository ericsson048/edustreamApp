  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ maxWidth: '80%', marginBottom: 8, padding: 10, borderRadius: 14, backgroundColor: '#1e293b', alignSelf: 'flex-start' }}>
            <ThemedText style={{ color: colors.primary, fontSize: 10, fontWeight: '700' }}>{item.sender_name || 'Member'}</ThemedText>
      ...
      {isMemberRef.current || joined ? (
        <View style={{ flexDirection: 'row', gap: 8, padding: Spacing.md, paddingBottom: insets.bottom + Spacing.md, borderTopWidth: 1, borderTopColor: colors.surfaceSecondary || '#1e293b' }}>
          <TextInput value={input} onChangeText={setInput} placeholder="Type a message..." placeholderTextColor="#64748b" style={{ flex: 1, backgroundColor: '#1e293b', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 10, color: '#fff', fontSize: 14 }} onSubmitEditing={handleSend} returnKeyType="send" />
          <TouchableOpacity onPress={handleSend} disabled={sending || !input.trim()} style={{ backgroundColor: sending ? '#334155' : '#2563eb', borderRadius: 14, paddingHorizontal: 16, justifyContent: 'center', opacity: !input.trim() ? 0.5 : 1 }}>
